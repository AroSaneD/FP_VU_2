...
maybeCharToString Nothing = "Nothing"
maybeCharToString ch = show ch
...
anyWinnersInBoard :: Moves -> Maybe Char
anyWinnersInBoard [] = Nothing
anyWinnersInBoard moves = anyWinners (getAllFinishedColsAndRows (getAllColumnsAndRow moves) [])
...
playGameAttack id pl moves =
    ...
    where
      ...
      url = buildUrl id pl
      freeCells = getFreeCells moves getAllCells []
      winner = anyWinnersInBoard moves
      winnerPost = anyWinnersInBoard offense
